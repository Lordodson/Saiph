...
namespace	//ANONYMOUS NAMESPACE: Global to this file only
{
	// TODO Factory Method Lab: Place your method definitions for object creation here
	BaseObject* CreateMouseCursor(BaseObject* mouseCur) {
		mouseCur = new BaseObject();
		ViewManager::GetInstance().AddObject(mouseCur, 5);
		mouseCur->SetTexture("Resources/images/reticle.png");
		mouseCur->SetDimensions(32.0f, 32.0f);
		mouseCur->SetColor(0.74f, 1.0f, 1.0f, 1.0f);
		return mouseCur;
	}
	BaseObject* CreateBackground() {
		BaseObject* bgrd = new BaseObject();
		ViewManager::GetInstance().AddObject(bgrd, 0);
		bgrd->SetTexture("Resources/images/background.png");
		bgrd->SetDimensions(1024.0f, 1024.0f);
		bgrd->SetPosition(512, 512);
		return bgrd;
	}
	PowerUp* CreatePowerUp(const int pwned) {
		PowerUp* powUp = new PowerUp();
		ViewManager::GetInstance().AddObject(powUp, 2);

		if (pwned == 0) { powUp->SetTexture("Resources/images/pupscore.png"); }
		else if (pwned == 1) { powUp->SetTexture("Resources/images/puparmor.png"); }
		else if (pwned == 2) { powUp->SetTexture("Resources/images/pupweapon.png"); }
		else if (pwned == 3) { powUp->SetTexture("Resources/images/pupenergy.png");	}
		return powUp;

		/*
		int randomPow = rand() % 4;
		powUp->SetType(randomPow);
		powUp->SetDimensions(32.0f, 32.0f);
		switch (powUp->GetType()) {
		case 0:	powUp->SCORE;	powUp->SetTexture("Resources/images/pupscore.png");
			break;
		case 1:	powUp->ARMOR;	powUp->SetTexture("Resources/images/puparmor.png");
			break;
		case 2:	powUp->WEAPON;	powUp->SetTexture("Resources/images/pupweapon.png");
			break;
		case 3:	powUp->ENERGY;	powUp->SetTexture("Resources/images/pupenergy.png");
			break;
		default:
			break;
		}
	...
	ProgressBar* CreateProgBar() {
		ProgressBar* ProgBar = new ProgressBar();
		ProgBar->SetTexture("Resources/images/progressBar2.png");
		ProgBar->SetDimensions(32.0f, 8.0f);
		ProgBar->SetPosition(0, 0);
		return ProgBar;
	}
...
void LevelState::GeneratePowerUps(Enemy *_enemy)
{
	...
	{
		PowerUp* pwned = CreatePowerUp(_enemy->GetPowerUpID());
		powerUps.push_back(pwned);
		...
		pwned->SetVelocity(randomVelocity);
		pwned->SetPosition(_enemy->GetShip()->GetPosition());
		pwned->SetDimensions(32.0f, 32.0f);
		/*
		if (_enemy->ARMOR < 50) {
			PowerUp* WeUp = CreatePowerUp();
			WeUp->SetType(3);
			WeUp->SetVelocity(randomVelocity);
			WeUp->SetPosition(_enemy->GetShip()->GetPosition());
			powerUps.push_back(WeUp);
		}
		else if (_enemy->ENERGY == 0) {
			PowerUp* EnUp = CreatePowerUp();
			EnUp->SetType(1);
			EnUp->SetVelocity(randomVelocity);
			EnUp->SetPosition(_enemy->GetShip()->GetPosition());
			powerUps.push_back(EnUp);

		}
		else if (_enemy->FLEE == 1) {
			PowerUp* FleeUp = CreatePowerUp();
			FleeUp->SetType(0);
			FleeUp->SetVelocity(randomVelocity);
			FleeUp->SetPosition(_enemy->GetShip()->GetPosition());
			powerUps.push_back(FleeUp);

		}
		else if (_enemy->ARMOR == 0) {
			PowerUp* ArUp = CreatePowerUp();
			ArUp->SetType(2);
			ArUp->SetVelocity(randomVelocity);
			ArUp->SetPosition(_enemy->GetShip()->GetPosition());
			powerUps.push_back(ArUp);
		}
		*/
	}
}
...
void LevelState::ResetPlayer()
{
	...
	playerShip = new MotherShip(*temp);
	ViewManager::GetInstance().RemoveObject(temp);
	delete temp;
	ViewManager::GetInstance().AddObject(playerShip, 1);
	...
	for (int i = 0; i < 5; i++) {
		Ship* freshlyMadeShadow = SaiphApp::CloneShipFromPrototype(SaiphApp::GetShipID(), 1);
		freshlyMadeShadow->SetColor(1.0f, 0.5f, 0.5f, 0.5f);
		((MotherShip*)playerShip)->ShadowAdd(freshlyMadeShadow);
	}
		...
		if (playerShip)
		{
			...
			if (GetAsyncKeyState('E') & 0x1) {
				if (playerShip->GetDeflector()) {
					...
					Deflector* def = new Deflector(playerShip, this);
					playerShip->SetDeflector(def);
				}
			}
			if (GetAsyncKeyState('F') & 0x1) {
				playerShip->GetDeflector()->ActivateStrategy("DLL");
			}
			if (GetAsyncKeyState('G') & 0x1) {
				playerShip->GetDeflector()->ActivateStrategy("DLL2");
			}
			...
			if (GetAsyncKeyState('C') & 0x1) {
				...
				for (x = 1; x < enemies.size(); ++x) {
					if (playerShip->GetBattery() >= 10) {
						if (enemies[x]->GetShip()->Collide(*cursor)) {
							GoBoom(enemies[x]);
			...
			if (GetAsyncKeyState('X') & 0x1) {
				static unsigned int x;
				for (x = 1; x < enemies.size(); ++x) {
					enemies[x]->NotifyObservers(Observer::ObserverEvent::ACTIVATE);
				}
			}
		}
	}
...
void LevelState::Init()
{
	...
	// progress bar
	enemyHealth = CreateProgBar();

	// mouse
	cursor = CreateMouseCursor(cursor);
...
void LevelState::GoBoom(Enemy* _enemy) {
	StickyBomb* b = new StickyBomb();
	b->SetPosition(playerShip->GetPosition());
	b->SetHeading(_enemy->GetShip()->GetHeading());
	b->SetTarget(_enemy);
	ViewManager::GetInstance().AddObject(b, 1);
	_enemy->Attach(b);
