...
void Ship::SetDeflector(Deflector* deflector) {
	defShip = deflector;
}
...
Deflector* Ship::GetDeflector() const { return defShip; }
...
bool Ship::Collide(const BaseObject& check) {
	return BaseObject::Collide(check);
}

bool Ship::Collide(Bullet& check) {

	bool  coll = false;
	if (!defShip) {
		coll = BaseObject::Collide(check);
	}
	else if (defShip->Collide(check)) {
		defShip->Deflect(&check);
		coll = false;
	}
	return coll;
}