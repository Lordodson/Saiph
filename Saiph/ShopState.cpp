...
{
	//ensured dim and col were using floats
	//changed layer number to unsigned int
	BaseObject *cur = new BaseObject();
	unsigned int layerNum = 5;
	ViewManager::GetInstance().AddObject(cur, layerNum);
	cur->SetTexture("Resources/images/reticle.png");
	cur->SetDimensions(30.0f,30.0f);
	menu.LinkCursor(cur);
	...
	cur->SetColor(0.5f,1.0f,0.0f,0.0f);
	return cur;
}
...
void ShopState::ResetPlayer()
{
	...
	//casted to float since Positions require float. Wondering if Xposition needs the same division.
	playerShip->SetYPosition( WINDOW_HEIGHT / 2.0f);
	//removed the negative for PI.
...
void ShopState::GarbageCollect()
{
	...
	{
		//seems like the if should be for if there are instead of if there aren't. removed '!'
		if(effects[loop]->GetActive())
		{
			ViewManager::GetInstance().RemoveObject(effects[loop]);
			effects.erase(effects.begin() + loop);
			//reordered delete to follow erase.
			delete effects[loop];
		}
		else ++loop;
	}
...
void ShopState::Init()
{
	...
	// menus
	SaiphApp::LoadVerticalMenuList("Resources/shop.xml", *currentMenu);
	submenu = "";
...
{
	char buffer[256];
	//Made sure floats were used
	ViewManager::GetInstance().BeginScene();
	ViewManager::GetInstance().RenderText(10.0f,20.0f,"Upgrade Ship Menu");
	ViewManager::GetInstance().RenderObjects();
	currentMenu->RenderItemText();
	scroll.RenderItemText();

	//Maybe Current score needs to be u intstead of d?
	sprintf(buffer, "Current Score: %.8u", SaiphApp::GetScore());
	ViewManager::GetInstance().RenderText(50.0f, 57.0f, buffer);
	sprintf(buffer, "With Selected: %.8d", alteredScore);

	if(alteredScore == (int)SaiphApp::GetScore())
		ViewManager::GetInstance().RenderText(50.0f, 72.0f, buffer);
	else if(alteredScore < (int)SaiphApp::GetScore())
		ViewManager::GetInstance().RenderText(50.0f, 72.0f, buffer, Color4f(1.0f, 1.0f, 0.0f, 0.0f));
	else
		ViewManager::GetInstance().RenderText(50.0f, 72.0f, buffer, Color4f(1.0f, 0.0f, 1.0f, 0.0f));

	// render hud text on top of everything else
	hud->Render();
	sprintf(buffer, "%.2f", playerShip->GetArmor());
	ViewManager::GetInstance().RenderText(725.0f, 54.0f, buffer, Color4f(1.0f,0.0f,1.0f,0.0f));
	sprintf(buffer, "%.2f", playerShip->GetShield());
	ViewManager::GetInstance().RenderText(725.0f, 107.0f, buffer, Color4f(1.0f,0.0f,1.0f,1.0f));
	sprintf(buffer, "%.2f", playerShip->GetAfterburner());
	ViewManager::GetInstance().RenderText(725.0f, 160.0f, buffer, Color4f(1.0f,1.0f,0.0f,0.0f));
	sprintf(buffer, "%.2f", playerShip->GetBattery());
	ViewManager::GetInstance().RenderText(725.0f, 213.0f, buffer, Color4f(1.0f,1.0f,1.0f,0.0f));
